...
nouserstr = 'avahi:x:107:108:Avahi mDNS daemon,,,:/var/run/avahi-daemon:/bin/false\nmessagebus:x:105:106::/var/run/dbus:/bin/false'
...
def get_users_for_shell(passwd_output: str = PASSWD_OUTPUT,
    ...
    userlist = []
    elements = []
    outputlist = passwd_output.split('\n')
    for i in range(len(outputlist)):
        elements = outputlist[i].split(':') 
        if elements[-1].endswith('/'+grep_shell):
            #pull first part of string and save to nameslist[]
            user = elements[0]
            userlist.append(user)    
    return userlist


ulist = get_users_for_shell()
print(ulist)
ulist = get_users_for_shell(nouserstr,'ksh')
print(ulist)
